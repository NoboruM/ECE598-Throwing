...
class PyBullet:
    def __init__(
        ...
        if gui:
            ...
            line_length = 1.0  # Length of axis lines
            p.addUserDebugLine([0,0,0], [line_length,0,0], [1,0,0], lineWidth=2)  # X-axis (Red)
            p.addUserDebugLine([0,0,0], [0,line_length,0], [0,1,0], lineWidth=2)  # Y-axis (Green)
...
class G1DualArm(FixedBaseRobot):
    def __init__(self, base_position=[0.0] * 3):
        ...
        f = os.path.join(cwd, "robot", "g1", "g1_dual_arm.urdf")
        print("f: ", f)
        self.urdf_filename = f
        super().__init__(f, base_position=base_position)
    def GetLinkState(self, link_index, compute_link_velocity, compute_fk):
